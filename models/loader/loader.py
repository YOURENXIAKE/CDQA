...
class LoaderCheckPoint:
    ...
    def _load_model(self, model_name):
        ...
        t0 = time.time()
        if 'bloom' in model_name: # 加载活字
            model = AutoModelForCausalLM.from_pretrained(model_name, use_safetensors=True).cuda()

            # configuration = BloomConfig.from_pretrained(model_name)
            # model = AutoModelForCausalLM.from_config(configuration).cuda()
            model = TensorParallelPreTrainedModel(model, ["cuda:0", "cuda:1"]) # 双卡推理
            tokenizer = AutoTokenizer.from_pretrained(model_name)
            # weights = {}
            # with safe_open(model_name+'/model.safetensors', framework="pt") as f:
            #     for key in f.keys():
            #         weights['transformer.'+key] = f.get_tensor(key)
            # # 通过safe_open()打开的权重缺少lm_head.weight这个权重
        ...
        if 'Qwen' in model_name: # 加载活字
            # Note: The default behavior now has injection attack prevention off.
            # tokenizer = AutoTokenizer.from_pretrained("/home/zrchen/yongyou/qwen70b/Qwen-72B-Chat-Int4", revision='master', trust_remote_code=True)

            # model = AutoModelForCausalLM.from_pretrained(
            #     "/home/zrchen/yongyou/qwen70b/Qwen-72B-Chat-Int4", revision='master',
            #     device_map="auto",
            #     trust_remote_code=True
            # ).eval()
            # return model, tokenizer

            from models.loader.vllm_wrapper import vLLMWrapper
            model = vLLMWrapper('/home/zrchen/yongyou/qwen70b/Qwen-72B-Chat', tensor_parallel_size=4,gpu_memory_utilization = 0.90,max_model_len=4000)
        ...
        if not any([self.llm_device.lower() == "cpu",
            ...
            if torch.cuda.is_available() and self.llm_device.lower().startswith("cuda"):
                ...
                if (num_gpus < 2 and self.device_map is None) or ('chatglm2' in model_name.lower()):
                    model = (
