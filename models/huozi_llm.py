...
class HuoZiLLM(BaseAnswer, LLM, ABC):
    ...
    top_p: float = 0.4
    top_k: int = 10
    repetition_penalty: float = 1.05 # 会导致模型输出无关项
    encoder_repetition_penalty: int = 1  # 会导致模型输出无关项
    min_length: int = 0
    logits_processor: LogitsProcessorList = None
    stopping_criteria: Optional[StoppingCriteriaList] = None
    eos_token_id: Optional[int] = [2]

    state: object = {'max_new_tokens': 500,
                     'seed': 1,
    ...
    def generatorAnswer(self,prompt: str,
        ...
        print(response)
        pattern = re.compile(r'^.+智能助手\s+(.+)用户\s+',re.DOTALL)
        res = pattern.match(response)
        if res is not None:
            response = res.group(1)  

        pattern = re.compile(r'^.+智能助手\s+(.+)系统\s+',re.DOTALL)
        res = pattern.match(response)
        if res is not None:
            response = res.group(1)  
        pattern = re.compile(r'^\s+(.+)系统\s+',re.DOTALL)
        res = pattern.match(response)
        if res is not None:
            response = res.group(1)   
        pattern = re.compile(r'^\s+(.+)用户\s+',re.DOTALL)
        res = pattern.match(response)
        if res is not None:
            response = res.group(1)           
